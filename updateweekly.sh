...
wget -O - -S --header="accept-encoding: gzip" https://www.edsm.net/dump/bodies7days.json | gzip -dc | json2csv/bin/json2csv.js -f "id","bodyId","name","type","subType","offset","distanceToArrival","isMainStar","isScoopable","age","spectralClass","luminosity","absoluteMagnitude","solarMasses","solarRadius","surfaceTemperature","orbitalPeriod","semiMajorAxis","orbitalEccentricity","orbitalInclination","argOfPeriapsis","rotationalPeriod","rotationalPeriodTidallyLocked","axialTilt","updateTime","systemId" > bodies7days.csv
...
if [ ! -f ./mysqlinfo.txt ]; then
	...
	mysql -u $mysqluser -p$mysqlpass -h $mysqlhost -D $mysqldb < updateweekly.sql
fi
